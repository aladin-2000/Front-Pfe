...
export class AlotTableViewModel {
  ...
  Select_last_wallet_group(p01_alod_azgrpf_t: string): Observable<{ data: any[]; columns: string[] }> {
    return this.alotService.Select_last_wallet_group(p01_alod_azgrpf_t).pipe(
      map((data: any[]) => {
        let columns: string[] = [];
        if (data.length > 0) {
          columns = Object.keys(data[0]);
